...
// Simple Card component with flip animation
function Card({ item, onClick, flipped }) {
  return (
    <div
      className={`relative w-20 h-20 perspective cursor-pointer ${
        flipped ? "cursor-default" : "cursor-pointer"
      }`}
      onClick={() => !flipped && onClick(item)}
    >
      <div
      ...
      >
        {/* Front */}
        <div
          className="absolute w-full h-full backface-hidden bg-neutral-800 rounded flex items-center justify-center text-3xl"
        ...
        {/* Back */}
        <div
          className="absolute w-full h-full backface-hidden bg-sky-500 rounded flex items-center justify-center text-3xl rotate-y-180"
...
function Leaderboard({ refresh }) {
  ...
  return (
    <div className="text-white mt-6">
      ...
      {scores.length === 0 ? (
        <p>No scores yet. Play the game to add your score!</p>
      ) : (
        <table className="table-auto text-left border-collapse border border-white mx-auto">
          <thead>
            <tr>
              <th className="border px-2 py-1">#</th>
              <th className="border px-2 py-1">Player</th>
              <th className="border px-2 py-1">Moves</th>
          ...
          <tbody>
            {scores.map((score, idx) => (
              <tr key={score.id}>
                ...
                <td className="border px-2 py-1">{score.username}</td>
                <td className="border px-2 py-1">{score.moves}</td>
                <td className="border px-2 py-1">{score.time_taken}s</td>
              </tr>
            ))}
          </tbody>
        </table>
    ...
    if (allMatched && cards.length > 0) {
      ...
      if (username) {
        axios
          ...
          .then(() =>
            // toggle to trigger leaderboard refresh
